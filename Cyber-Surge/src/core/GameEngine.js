...
export class GameEngine {
            constructor(container) {
                ...
                this.isPaused  = false;
                this.gameState = 'idle'; // 'idle' | 'playing' | 'over'
                this.deltaTime   = 0;
                this.elapsedTime = 0;
                this.gameTime    = 0;
            ...
            handleTouchEnd(e) {
                if (!this.touchStarted || !this.isRunning || this.isPaused || this.gameState !== 'playing') return;
                ...
                if (Math.abs(deltaX) > Math.abs(deltaY)) {
                    if (deltaX < -25) this.player.moveLeft();
                    else if (deltaX > 25) this.player.moveRight();
                } else {
                    if (deltaY < -25) this.player.jump();
                    else if (deltaY > 25) this.player.slide();
                }
            ...
            update() {
                ...
                this.currentSpeed = Math.min(this.config.baseSpeed * speedMultiplier, this.config.maxSpeed);
