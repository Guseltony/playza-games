...
bkcore.hexgl.HexGL.prototype.displayScore = function(f, l)
{
	...
	var finalScore = 0;
	var adrenalineBonus = Math.floor(this.adrenaline * 10000);
	var perfectLapMultiplier = this.perfectLap ? 1.5 : 1.0;

	if (this.gameplay.result === this.gameplay.results.FINISH) {
		finalScore = Math.max(10000, 1000000 - Math.floor(f * 2));
		finalScore = Math.floor((finalScore + adrenalineBonus) * perfectLapMultiplier);
	} else {
		var survivedBonus = Math.floor(f / 100); 
		finalScore = Math.max(0, (5000 * this.gameplay.lap) + survivedBonus + adrenalineBonus);
	}

	// Final Payload Submittal
	window.parent.postMessage({
		type: "PLAYZA_SCORE_SUBMISSION",
		payload: { score: finalScore }
	}, "*");

	// We no longer need to display the local DOM step-5 gameover since React is overlaying GameOverLeaderboard
	/*
