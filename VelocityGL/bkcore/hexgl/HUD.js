...
bkcore.hexgl.HUD.prototype.update = function(speed, speedRatio, shield, shieldRatio)
{
	...
	if(!this.bg || this.bg.width == 0) return;
	var w = this.bg.width;
	...
	if(this.step == 0)
	{
		...
		if(!this.messageOnly)
		{
			...
			if (this.fgspeed && this.fgspeed.width > 0) {
				this.ctx.save();
				this.ctx.beginPath();
			...
			if (this.fgshield && this.fgshield.width > 0) {
				this.ctx.save();
				this.ctx.beginPath();
				...
				this.ctx.lineTo(-sw+SCREEN_HW, oh+sh+sy);
				this.ctx.lineTo(-sw+SCREEN_HW, oh+sy); // FIXED: close path correctly
				this.ctx.clip();
	...
	else if(this.step == 1)
	{
		...
		// Setup default drop shadow for readability
		this.ctx.shadowBlur = 10;

		// TIME
	    if(this.time != "")
	    {
			this.ctx.font = "bold " + (SCREEN_WIDTH/this.timeFontRatio * 1.2) + "px " + this.font;
		    this.ctx.fillStyle = "#06b6d4"; // Playza Cyan
		...
		if(this.lap != "")
		{
			this.ctx.font = "bold " + (SCREEN_WIDTH/this.timeFontRatio * 1.3) + "px " + this.font;
		    this.ctx.fillStyle = "#a855f7"; // Playza Purple
		    this.ctx.shadowColor = "rgba(168, 85, 247, 0.6)";
		    this.ctx.textAlign = "right";
		    this.ctx.fillText("LAP " + this.lap, SCREEN_WIDTH - (SCREEN_WIDTH/this.lapMarginRatio), SCREEN_WIDTH/this.timeMarginRatio);
		    this.ctx.textAlign = "center";
		}

		// PLAYZA BRANDING
		this.ctx.font = "italic bold " + (SCREEN_WIDTH / (this.timeFontRatio * 1.1)) + "px " + this.font;
		this.ctx.fillStyle = "#ffffff";
		this.ctx.shadowColor = "rgba(255, 255, 255, 0.4)";
		this.ctx.textAlign = "left";
		this.ctx.fillText("PLAYZA WARZONE", SCREEN_WIDTH/this.lapMarginRatio, SCREEN_WIDTH/this.timeMarginRatio);
		this.ctx.textAlign = "center";

		// Reset shadow
		this.ctx.shadowBlur = 0;

		// PLAYZA ADRENALINE HUD UI
		if(speedRatio > 0.6) {
			this.ctx.font = "bold " + (SCREEN_WIDTH/40) + "px " + this.font;
			this.ctx.fillStyle = "rgba(250, 204, 21, 0.8)"; // Yellow
			this.ctx.fillText("ADRENALINE FLOWING", SCREEN_HW, SCREEN_HEIGHT - (nh * 0.8));
		}
