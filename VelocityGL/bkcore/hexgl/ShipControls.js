...
bkcore.hexgl.ShipControls = function(ctx)
{
	...
	// PLAYZA ADRENALINE & SKILL TRACKING
	this.adrenaline = 1.0;
	this.adrenalineTimer = 0;
	this.perfectLap = true; // Flips to false if shield takes any damage
	...
	if(ctx.controlType == 1 && bkcore.controllers.TouchController.isCompatible())
	{
		// Virtual Pad Logic
		var _pad = {
			l: domElement.getElementById('pad-left'),
			r: domElement.getElementById('pad-right'),
			g: domElement.getElementById('pad-gas'),
			b: domElement.getElementById('pad-brake')
		};

		if(_pad.l) {
			_pad.l.ontouchstart = function() { self.key.left = true; };
			_pad.l.ontouchend = function() { self.key.left = false; };
			_pad.r.ontouchstart = function() { self.key.right = true; };
			_pad.r.ontouchend = function() { self.key.right = false; };
			_pad.g.ontouchstart = function() { self.key.forward = true; };
			_pad.g.ontouchend = function() { self.key.forward = false; };
			_pad.b.ontouchstart = function() { self.key.backward = true; self.key.ltrigger = true; };
			_pad.b.ontouchend = function() { self.key.backward = false; self.key.ltrigger = false; };
		}
	}
...
bkcore.hexgl.ShipControls.prototype.update = function(dt)
{
	...
	// PLAYZA ADRENALINE CALCULATOR
	if(this.speed > this.maxSpeed * 0.8) {
		this.adrenalineTimer += dt;
		if(this.adrenalineTimer > 60) { // Every second at high speed
			this.adrenaline = Math.min(5.0, this.adrenaline + 0.1);
			this.adrenalineTimer = 0;
		}
	} else {
		this.adrenaline = Math.max(1.0, this.adrenaline - 0.05 * dt);
	}
...
bkcore.hexgl.ShipControls.prototype.collisionCheck = function(dt)
{
	...
	if(collision.r < 255)
	{
		this.perfectLap = false; // VOID PERFECT LAP BONUS
